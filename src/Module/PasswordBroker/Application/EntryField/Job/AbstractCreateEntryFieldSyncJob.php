...
abstract class AbstractCreateEntryFieldSyncJob extends AbstractReplicableSyncJob
{
    ...
    public const string PAYLOAD_KEY_ENTRY_ID = EntryFieldRepository::FIELD_ENTRY_ID;
    ...
    public function handle(): AbstractEntryField
    {
        ...
        $entry = $entryRepository->findById(
            EntryFieldId::fromRaw($this->payload[self::PAYLOAD_KEY_ENTRY_ID]),
        );
        if (!$entry) {
    ...
    protected function validate(): void
    {
        ...
        if (empty($this->payload[self::PAYLOAD_KEY_ENTRY_ID])) {
